...
import com.iec.solr.bean.ProductGold;
...
public class ProductGoldService {
	...
	private static Logger logger = Logger.getLogger(ProductGoldService.class);

	
	public static  String url_product_gold=PropsConfig.getPropValue("url_product_gold",null);
	
	private @Autowired ProductGoldDao assetManageDao;
	...
	public boolean addAssetManageIndex(String solrUrl,String sql,int currentpage,int pagesize,String[] visibles) throws ServiceException, SolrServerException, IOException{
		...
		logger.info("实物金查询将要删除数据的长度:"+listMap.size());
...
//				AnalyXML.updateNewTime(date, "t_product_gold");
		    	SolrConfig.setPropValue("t_product_gold", date);
				ProductGoldAddTimer.t_product_gold = date;
				logger.info("实物金成功删除索引的长度:"+listMap.size());
		    	return true;
		    }else{
		       return false;
		    }
	...
	public boolean deleteAssetManageIndex(String solrUrl,String sql,int currentpage,int pagesize) throws ServiceException, SolrServerException, IOException{
		...
		logger.info("实物金查询将要删除数据的长度:"+goodsList.size());
...
//				AnalyXML.updateNewTime(date, "t_product_gold_delete");
				SolrConfig.setPropValue("t_product_gold_delete", date);
				ProductGoldDeleteTimer.t_product_gold_delete=date;
				logger.info("实物金成功删除索引的长度:"+goodsList.size());
				return true;
			}else{
				return false;
			}
	...
	public Map<String,Object> queryProductList(String params, String column,String sort,Integer current,Integer pagesize){
		List<ProductGold> list=null;
		Map<String,Object> fmap = new HashMap<String,Object>();//返回给用户Map的对象
		solrimpl.init(url_product_gold); //solr初始化
		...
		try {
			...
			list = qs.getBeans(ProductGold.class);
	...
	public Map<String,Object> queryAssetManage(Map<String,String> params,Map<String,String> sort,Map<String,String> page) throws ServiceException, IllegalAccessException, InvocationTargetException {
		...
		Map<String,Object> fmap = new HashMap<String,Object>();//返回给用户Map的对象
		solrimpl.init(ProductGoldAddTimer.t_product_gold); //solr初始化
